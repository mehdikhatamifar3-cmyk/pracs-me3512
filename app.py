...
def preserve_widget_values_across_pages() -> None:
    """Detach page-specific widget values from Streamlit's widget cleanup."""
...
def active_conduction_data() -> pd.DataFrame:
    ...
    return normalise_conduction_data(st.session_state.conduction_data)


def active_radiation_data() -> pd.DataFrame:
    if online_mode_active():
        return assigned_online_radiation_data(st.session_state.student_id)
    return normalise_radiation_data(st.session_state.radiation_data)
...
def replace_conduction_data(data: pd.DataFrame) -> None:
    st.session_state.conduction_data = normalise_conduction_data(data)
    st.session_state.cond_editor_version += 1


def replace_radiation_data(data: pd.DataFrame) -> None:
    st.session_state.radiation_data = normalise_radiation_data(data)
...
def render_conduction_record() -> None:
    ...
    if online_mode_active():
        ...
        physical_data = normalise_conduction_data(st.session_state.conduction_data)
        ...
        st.session_state.conduction_data = normalise_conduction_data(edited)
    ...
    if online_mode_active():
        ...
        with b2:
            if st.button("Reset to blank table", use_container_width=True, key="reset_rad"):
                ...
                st.rerun()
        st.caption("Enter only your group's measured values. The table is retained when you move between workflow sections.")
        physical_data = normalise_radiation_data(st.session_state.radiation_data)
        edited = st.data_editor(
            physical_data,
        ...
        st.session_state.radiation_data = normalise_radiation_data(edited)
        valid = valid_radiation_rows(st.session_state.radiation_data)
        incomplete = len(edited) - len(valid)
    if len(valid):
        st.markdown(
...
def render_conduction_explore() -> None:
    ...
    with left:
        q_flux = st.slider("Heat flux q'' (kW/m²)", 2.0, 80.0, step=1.0, key="cond_sandbox_q_flux")
        r_hot_micro = st.slider("Hot contact R'' (×10⁻⁴ m²·K/W)", 0.0, 20.0, step=0.2, key="cond_sandbox_r_hot")
        r_cold_micro = st.slider("Cold contact R'' (×10⁻⁴ m²·K/W)", 0.0, 30.0, step=0.2, key="cond_sandbox_r_cold")
        conductivity = st.slider("Sample conductivity k (W/m·K)", 20, 240, step=5, key="cond_sandbox_k")
        sample_length_mm = st.slider("Sample length L (mm)", 10, 60, step=1, key="cond_sandbox_length")
...
def render_radiation_explore() -> None:
    ...
    with left:
        air_c = st.slider("True air temperature Tm (°C)", 0.0, 80.0, step=1.0, key="rad_sandbox_air")
        wall_c = st.slider("Surrounding wall temperature Tsur (°C)", 0.0, 250.0, step=5.0, key="rad_sandbox_wall")
        h = st.slider("Convection coefficient h (W/m²·K)", 2.0, 500.0, step=2.0, key="rad_sandbox_h")
        emissivity = st.slider("Bead emissivity ε", 0.05, 1.00, step=0.01, key="rad_sandbox_emissivity")
